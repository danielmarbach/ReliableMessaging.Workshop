...
public class Processor : IHostedService
{
    ...
    public Processor(ProcessorOptions processorOptions, BlobContainerClient blobContainerClient, BatchProcessor batchProcessor, ILogger<Processor> logger)
    {
    ...
    {
        await RestartFromBeginningIfNecessary(processorOptions.RestartFromBeginning, blobContainerClient);
        ...
        batchProcessor.ProcessEventAsync += events =>
        {
            foreach (var @event in events)
            {
                ...
                var numberOfDataPointsObserved = channelObservations.AddOrUpdate(channel,
                    ...
                    (CurrentTemperature: temperatureChanged.Current, TemperatureThreshold: processorOptions.TemperatureThreshold));

                logger.LogInformation(numberOfDataPointsObserved < processorOptions.NumberOfDataPointsToObserve
                    ? $" - {channel}: {temperatureChanged.Current} / {temperatureChanged.Published}{(numberOfDataPointsObserved == 0 ? $" (below threshold of '{processorOptions.TemperatureThreshold}', reset)" : string.Empty)}"
                    : $" - {channel}: {temperatureChanged.Current} / {temperatureChanged.Published} (above threshold of '{processorOptions.TemperatureThreshold}' for the last '{numberOfDataPointsObserved}' data points)");
            }
